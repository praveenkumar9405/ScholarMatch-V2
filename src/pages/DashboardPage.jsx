...
import { Loader2, Search, UserCircle2, Sparkles, LogOut, Wallet, Award, ArrowUpRight } from 'lucide-react';
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
           <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: 0.1}} className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex flex-col justify-between overflow-hidden relative group">
              ...
              <div>
                <p className="text-text/60 font-bold tracking-wide text-sm uppercase mb-1">Eligible Matches</p>
           ...
           <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: 0.2}} className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.03)] flex flex-col justify-between overflow-hidden relative group">
              ...
              <div>
                <p className="text-text/60 font-bold tracking-wide text-sm uppercase mb-1">Potential Funding</p>
           ...
           <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{delay: 0.3}} className="bg-gradient-to-br from-gray-900 to-black rounded-[2.5rem] p-8 border border-gray-800 shadow-[0_8px_30px_rgb(0,0,0,0.1)] flex flex-col justify-between overflow-hidden relative group text-white">
              ...
              <div>
                <p className="text-white/60 font-bold tracking-wide text-sm uppercase mb-1">Profile Strength</p>
                <div className="flex items-end gap-3">
                  <h3 className="text-4xl font-extrabold tracking-tight text-white">100%</h3>
                  <span className="text-sm font-semibold bg-white/20 px-3 py-1 rounded-full mb-1">Verified</span>
                </div>
              </div>
           </motion.div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
           <div className="pl-2 border-l-4 border-primary">
              <h2 className="text-3xl font-extrabold tracking-tight">Your Pipeline</h2>
           </div>
           <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="flex items-center p-1 bg-gray-200/50 rounded-full shrink-0 w-full sm:w-auto">
                 <button onClick={() => setTab('all')} className={`flex-1 sm:flex-none px-6 py-2 rounded-full text-sm font-bold transition-all ${tab === 'all' ? 'bg-white shadow-sm text-black' : 'text-gray-500 hover:text-black'}`}>Latest Matches</button>
                 <button onClick={() => setTab('saved')} className={`flex-1 sm:flex-none px-6 py-2 rounded-full text-sm font-bold transition-all ${tab === 'saved' ? 'bg-white shadow-sm text-black' : 'text-gray-500 hover:text-black'}`}>Saved & Tracked</button>
              </div>
           </div>
        </div>
        ...
        {(() => {
          ...
          return (
            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-32 bg-white rounded-[3rem] border border-gray-200 shadow-[0_8px_30px_rgb(0,0,0,0.03)] px-6">
              ...
              <h3 className="text-3xl font-extrabold text-text mb-4 tracking-tight">No precise matches yet</h3>
              <p className="text-text/60 max-w-lg text-lg mx-auto leading-relaxed font-medium">We couldn't find a scholarship that fits this exact filter context right now. Try adjusting your search term!</p>
              <button onClick={() => { setSearchQuery(''); setTab('all'); }} className="mt-8 px-8 py-4 bg-black text-white rounded-full font-bold shadow-xl hover:scale-105 transition-transform">Clear Filters</button>
            </motion.div>
          );
        })()}
      </main>
